...
# it is set to send a message to only Jealhyn,  but you can change the if "<>"
...
def message_sent(event_data):
    ...
    if "<@U02QW5R881Z>" in message.get("text"):
...
def my_first_slash_command():
    ...
    # Creating an authorization token and server to then be able to sign-in.
        tess_auth=TSC.PersonalAccessTokenAuth(tess_token,tess_secret,
                                              site_id = tess_domain)
    ...
    # getting infromation from.
        with tess_serv.auth.sign_in(tess_auth):
            utilization = tess_serv.views.get_by_id('5c41cd96-6263-447f-93c9-1cba0cb56348')
            tess_serv.views.populate_image(utilization)
            
            # Storing that image into my local computer.
            with open(tess_path,'wb') as v:
        ...
        blocksjson=[
                ...
                },
                {
                     ...
                     "elements": [
                        {
                          "type": "mrkdwn",
                          "text": "Link to  <https://prod-useast-b.online.tableau.com/#/site/tessellationhq/views/ProjectReportingSLACK/BillableUtilizationforSlack>"
                        }
                      ]
                }
            ]
